...
<script setup>
    ...
    if (route.query.join) {
        ...
        // Show the name changer modal with join hint
        console.log('💡 Join code detected, showing name modal:', joinCode.value)
        }
    }
    })

    function handleNameSaved(data) {
    console.log('✅ Name saved:', data.username)

    // If there's a join code, auto-join the lobby
    if (data.joinCode) {
        console.log('🚀 Auto-joining lobby:', data.joinCode)
